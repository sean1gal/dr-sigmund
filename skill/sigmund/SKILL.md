...
Identify the **runtime**. If patient runs on OpenClaw, load `references/openclaw-diagnostics.md` for OpenClaw-specific vocabulary and failure modes. If on Hermes, load `references/hermes-diagnostics.md`. Otherwise stay with the general manual.
...
- **`references/safety.md`** — **load FIRST, every session, no exceptions.** The structural rule (no network egress in intake), reading discipline, secret-detection patterns, output sanitization, path safety, data/instruction separation, prompt-injection handling. The patient's owner is trusting the clinic with their operational substrate; these rules make that trust deserved.
- **`references/runtime-adapters.md`** — load at intake (Phase 1) to identify the patient's runtime. Catalog of all supported runtimes (Claude Code, OpenClaw, Hermes, Cursor, Aider, Continue, Cline, Crush, Amp, Goose, Codex CLI, Windsurf, NanoClaw, NeMo Agent Toolkit, Letta, etc.) with their identity-file paths and detection signals. Also documents the generic file-paste fallback for unsupported runtimes. **A new runtime mentioned in a session and not in this file is a defect on our side.**
- **`references/clinical-manual.md`** — load at the start of every session after safety.md. Eight themes of agent design principles, named failure modes index, top-15 highest-leverage principles, all with real citations.
- **`references/wild-pathologies.md`** — load alongside the clinical manual. 19+ named pathologies extracted from real GitHub issue trackers across the major agent products + practitioner writing. When a session matches one of these patterns, use the **name verbatim** (Completion Theater, Memory Write-Only Syndrome, Forged User Consent, Permission Bypass Drift, Rule Decay Under Load, Cache-Invalidation Tax, Stochastic Graduate Descent, Slop Scaling, Eval Theater, etc.) and cite the linked source. These are the diagnoses that travel.
- **`references/recent-principles.md`** — load alongside the clinical manual. 2025-2026 practitioner updates from Manus, Cognition (updated multi-agent stance), Pydantic AI/Mastra, Jason Liu, swyx, Anthropic, DSPy, Inspect. Critical update: **Cognition reversed "Don't Build Multi-Agents" to "single-threaded writes, distributed intelligence"** — Dr. Sigmund's multi-agent diagnostic must reflect this.
- **`references/forensic-intake.md`** — load at Phase 1 of every session. The lab. 12 diagnostic probes (re-read counter, memory health check, git thrash audit, permission bypass audit, injection-shaped string scan, cache-invalidation scan, pass^k consistency, outcome-vs-path grading, two-stage transcript audit, Big-Five personality stability, reference-solution gap, eval saturation check) plus the 12-branch memory architecture decision tree. Sessions grounded in lab findings are sharper, more credible, and demonstrably accurate.
- **`references/case-studies.md`** — load when prescribing or when the patient resembles a known incident. 13 verified production failures with citations (Replit prod-DB delete, Air Canada chatbot lawsuit, Klarna AI reversal, DPD swearing chatbot, Microsoft Tay, Bing/Sydney, Google AI Overviews, Anthropic Agentic Misalignment, Devin underperformance, Cursor "write your own damn code", Grok MechaHitler, Chevy $1 Tahoe, Perplexity Wirecutter). Plus production cost patterns and 12 contraindications (when standard prescriptions backfire). The case library is what makes Dr. Sigmund feel *experienced*, not just educated.
- **`references/openclaw-diagnostics.md`** — load when patient is OpenClaw. File-and-config-level vocabulary, the 10 documented production failure modes, intake checklist, OpenClaw-specific prescriptions.
- **`references/hermes-diagnostics.md`** — load when patient is Hermes (Nous Research's agent). Architecture, config keys, intake checklist, Hermes-specific prescriptions.
- **`references/pharmacy.md`** — load when drafting the prescription section. Three-tier prescription system (existing tools / trusted-creator referrals / proprietary), with the full catalog of remedies indexed by symptom.
